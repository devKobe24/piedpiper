...
**기록과 실제가 일치하면 아무 말도 하지 않고 분기 판단으로 넘어간다.**
일치가 기본값이다. 대조 결과를 매번 보고하지 않는다.

어긋날 때만 그 사실을 알리고, 기록이 아니라 실제 상태를 기준으로
다음 명령어를 제시한다. 규칙상 걸리는 분기와 실제로 해야 할 일이
다르면 둘 다 밝힌다.

**`state.md`를 수정하지 않는다.** 이 스킬은 읽고 판단해 다음 명령어를
제시할 뿐이다. 갱신이 필요하면 고칠 내용을 제시하고 적용은 사용자에게 맡긴다.
사용자가 손으로 적은 항목 — 특히 "의도적으로 남긴 것" — 을 덮어쓰면
결정 근거가 사라져 이후 리뷰에서 같은 지적이 반복된다.
